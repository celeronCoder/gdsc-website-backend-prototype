      ...
      <div className="flex flex-col items-center justify-center p-10">
        ...
        {user?.user ? (
          <div className="items-right flex w-full justify-end">
            <button
              ...
              onClick={async () => void (await router.push("/events/create"))}
            >
              Create New Event
            </button>
          </div>
        ...
        <div className="mt-10 flex flex-col items-center justify-around gap-10">
          {data.map((event) => (
            <div
              key={event.id}
              className="flex flex-col items-start justify-start gap-1 rounded border-2 border-black py-2 px-4 shadow-md"
            >
